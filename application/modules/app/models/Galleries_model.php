...
class Galleries_model extends CI_Model {
    ...
    // get data by slug
    function get_by_slug($slug, $status = array('poststatus !=' => 4)) {
        ...
        $this->db->where('posts.type', 'gallery');
    ...
    // get total rows
    function total_rows($whereby = array(), $q = NULL, $status = array('poststatus !=' => 4)) {
        $this->db->select('posts.*, posts.subtype');
        $this->db->from('posts');
        $this->db->where('posts.type', 'gallery');
        $this->db->where($status);
        if (!empty($whereby) && is_array($whereby)) {
        ...
        if ($q) {
            ...
            $this->db->or_like('posts.subtype', $q);
            $this->db->or_like('posts.postimage', $q);
            $this->db->or_like('posts.postimagethumb', $q);
            $this->db->group_end();
        }
    ...
    function get_limit_data($whereby = array(), $limit, $start = 0, $q = NULL, $status = array('poststatus !=' => 4)) {
        $this->db->select('posts.*, posts.subtype');
        $this->db->from('posts');
        $this->db->where('posts.type', 'gallery');
        $this->db->where($status);
