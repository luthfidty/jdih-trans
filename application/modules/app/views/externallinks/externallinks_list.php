<?php
// contoh di controller/view tertentu
$this->load->view('base/headercontent'); // atau: include APPPATH.'views/includes/header.php';
?>
<!-- /Page Title -->


<div class="space-y-6">
    <div class="bg-white border border-gray-200 rounded-lg shadow">
        <!-- Panel Heading -->
        <div class="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center gap-4">
            <!-- Left Buttons -->
            <div class="flex flex-wrap">
                <?php if (in_array('create', $rolelist)): ?>
                    <?= $this->myacl->_btnCreate(site_url('app/externallinks/create'), "Tambah") ?>
                <?php endif; ?>
            </div>

            <!-- Center Message -->
            <!-- <div class="text-sm text-gray-600 text-center flex-1">
                <?php echo $this->session->userdata('message') ?: ''; ?>
            </div> -->

            <!-- Search Form -->
            <form action="<?php echo site_url('app/externallinks/index'); ?>" method="get"
                class="flex gap-2 items-center">
                <input type="text" name="q" value="<?php echo $q; ?>" placeholder="Cari..."
                    class="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                <?php if ($q <> ''): ?>
                    <a href="<?php echo site_url('app/externallinks'); ?>"
                        class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm">Atur Ulang</a>
                <?php endif; ?>
                <button type="submit"
                    class="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600 transition">Cari</button>
            </form>
        </div>

        <!-- Panel Body / Table -->
  <div class="relative overflow-x-auto">
    <table class="min-w-full text-sm text-left border-t border-gray-200 table-fixed">
        <thead class="bg-gray-100 text-gray-700 sticky top-0 z-20">
            <tr>
                <th class="sticky left-0 z-30 bg-gray-100 px-4 py-2 border-b w-[60px]">No</th>
                <th class="px-4 py-2 border-b">Logo</th>
                <th class="px-4 py-2 border-b">Nama Situs</th>
                <th class="px-4 py-2 border-b">Alamat URL</th>
                <th class="sticky right-0 z-30 bg-gray-100 px-4 py-2 border-b text-center w-[120px]">Aksi</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($externallinks_data as $externallinks): ?>
                <tr class="border-b hover:bg-gray-50">
                    <td class="sticky left-0 z-10 bg-white px-4 py-2 w-[60px]"><?php echo ++$start ?></td>
                    <td class="px-4 py-2">
                        <?php if (!empty($externallinks->postimage)): ?>
                            <a href="<?php echo site_url($externallinks->postimage) ?>" target="_blank" class="block w-24">
                                <img src="<?php echo site_url($externallinks->postimagethumb) ?>" class="rounded border" alt="<?php echo $externallinks->title ?>" />
                            </a>
                        <?php endif; ?>
                    </td>
                    <td class="px-4 py-2"><?php echo $externallinks->title ?></td>
                    <td class="px-4 py-2">
                        <a href="<?php echo $externallinks->content ?>" target="_blank" class="inline-block bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 transition">
                            <i class="fas fa-paper-plane"></i> Kunjungi
                        </a>
                    </td>
                    <td class="sticky right-0 z-10 bg-white px-4 py-2 text-center w-[120px]">
                        <div class="flex justify-center gap-2">
                            <?php if (in_array('update', $rolelist)) { ?>
                                <a href="<?php echo site_url('app/externallinks/update/' . $externallinks->id) ?>"
                                    class="w-9 h-9 flex items-center justify-center rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-800 transition duration-200"
                                    title="Edit">
                                    <i class="fas fa-edit text-sm"></i>
                                </a>
                            <?php } ?>
                            <?php if (in_array('delete', $rolelist)) { ?>
                                <a href="<?php echo site_url('app/externallinks/delete/' . $externallinks->id) ?>"
                                    onclick="return confirm('Yakin ingin menghapus?')"
                                    class="w-9 h-9 flex items-center justify-center rounded-full bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-800 transition duration-200"
                                    title="Hapus">
                                    <i class="fas fa-trash-alt text-sm"></i>
                                </a>
                            <?php } ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
</div>

        <!-- Panel Footer -->
        <div class="px-4 py-3 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center">
            <div class="text-sm text-gray-600 mb-2 md:mb-0">
                Jumlah Data: <strong><?php echo $total_rows ?></strong>
            </div>
            <div>
                <?php echo $pagination ?>
            </div>
        </div>
    </div>
</div>