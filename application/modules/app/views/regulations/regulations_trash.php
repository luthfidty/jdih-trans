    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow overflow-hidden">
        ...
        <div class="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <?php
                echo $this->myacl->_btnDefault(site_url('app/regulations'), "Daftar Peraturan", "bg-cyan-600 hover:bg-cyan-700", "fa-list");
        ...
        <!-- Table Responsive -->
        <form id="bulkDeleteForm" action="<?php echo site_url('app/regulations/bulk_permanent_delete'); ?>" method="post">
                ...
                <table class="min-w-full text-sm text-left border-t border-gray-200 table-fixed">
                    <thead class="bg-gray-100 text-gray-700 sticky top-0 z-20">
                        <tr>
                            ...
                            <th class="px-4 py-2 border-b whitespace-nowrap">Tgl Pengundangan</th>
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                <td class="sticky left-0 z-10 bg-white px-4 py-2 w-[40px]">
                                    <?php if (in_array('permanent_delete', $rolelist)): ?>
                                        <input type="checkbox" name="ids[]" value="<?php echo $regulations->id; ?>" class="rowCheckbox">
                                    <?php endif; ?>
                                </td>
                                <td class="sticky left-[40px] z-10 bg-white px-4 py-2 w-[60px]"><?php echo ++$start ?></td>
                                <td class="sticky left-[100px] z-10 bg-white px-4 py-2 w-[200px]"><?php echo $regulations->title ?></td>
                                ...
                                <td class="px-4 py-2 tanggal" data-date="<?php echo $regulations->assignmentdate ?>"
                                    data-format="DD/MM/YYYY"></td>
                                <td class="px-4 py-2 tanggal" data-date="<?php echo $regulations->approvaldate ?>"
                                    data-format="DD/MM/YYYY"></td>
                                <td class="px-4 py-2 tanggal" data-date="<?php echo $regulations->effectivedate ?>"
                                    data-format="DD/MM/YYYY"></td>
                                ...
                                <td class="sticky right-0 z-10 bg-white px-4 py-2 text-center w-[120px]">
                                    <div class="flex justify-center gap-2">
                                        <?php if (in_array('restore', $rolelist)): ?>
                                            <a href="<?php echo site_url('app/regulations/restore/' . $regulations->id) ?>" title="Pulihkan"
                                        ...
                                        <?php endif; ?>
                                        <?php if (in_array('permanent_delete', $rolelist)): ?>
                                            <a href="<?php echo site_url('app/regulations/permanent_delete/' . $regulations->id) ?>"
        ...
        <div class="px-4 py-3 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center">
            <div class="flex items-center gap-2 mb-2 md:mb-0">
                <form action="<?php echo site_url('app/regulations/trash'); ?>" method="get" class="flex items-center">
                    <select name="per_page" id="per_page" class="border border-gray-300 rounded px-2 py-1 text-sm"
                        ...
                        foreach ($page_sizes as $size): ?>
                            <option value="<?php echo $size ?>" <?php echo isset($_GET['per_page']) && $_GET['per_page'] == $size ? 'selected' : '' ?>>
                                <?php echo $size ?> per halaman
                            </option>
                        <?php endforeach; ?>
                    </select>
