...
<div class="bg-white shadow rounded-lg p-6">
    <form method="post" action="<?php echo $action ?>" autocomplete="off" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Kunci Kategori -->
            <div>
                <label for="group_key" class="block font-medium text-gray-700">
                    Kunci Kategori <?php echo form_error('group_key') ?>
                </label>
                <input type="text" name="group_key" id="group_key" placeholder="Kunci Kategori"
                    value="<?php echo $group_key; ?>"
                    class="mt-1 w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-500" />
            </div>

            <!-- Nama Kategori -->
            <div>
                <label for="group_name" class="block font-medium text-gray-700">
                    Nama Kategori <?php echo form_error('group_name') ?>
                </label>
                <input type="text" name="group_name" id="group_name" placeholder="Nama Kategori"
