...
<div class="space-y-6">
    <div class="bg-white border border-gray-200 rounded-lg shadow overflow-hidden">
        ...
        <div class="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center gap-4">
            ...
            <div class="flex flex-wrap">
                ...
                if (in_array('create', $rolelist)) {
                    echo $this->myacl->_btnCreate(site_url('app/galleries/create'), "Tambah");
                    echo $this->myacl->_btnDefault(site_url('app/galleries/trash'), "Tong Sampah", "bg-teal-600 hover:bg-teal-700", "fa-trash");
                }
            ...
            <!-- Search Form -->
            <form action="<?php echo site_url('app/galleries/index'); ?>" method="get"
        ...
        <!-- Table Responsive -->
        <form id="bulkDeleteForm" action="<?php echo site_url('app/galleries/bulk_delete'); ?>" method="post">
                    ...
                    <tbody>
                        <?php foreach ($galleries_data as $galleries): ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    <a href="<?php echo site_url($galleries->postimage) ?>" target="_blank"
                                        class="block w-24">
                                        <img src="<?php echo site_url($galleries->postimagethumb) ?>" class="rounded border"
                                            alt="<?php echo $galleries->title ?>" />
                                    </a>
                                </td>
                                ...
                                <td class="sticky right-0 z-10 bg-white px-4 py-2 text-center w-[120px]">
                                    <div class="flex justify-center gap-2">
                                        <?php if (in_array('update', $rolelist)) { ?>
                                            <a href="<?php echo site_url('app/galleries/update/' . $galleries->id) ?>"
                                        ...
                                        <?php } ?>
                                        <?php if (in_array('delete', $rolelist)) { ?>
                                            <a href="<?php echo site_url('app/galleries/delete/' . $galleries->id) ?>"
        ...
        <div class="px-4 py-3 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center">
            <div class="flex items-center gap-2 mb-2 md:mb-0">
                <form action="<?= site_url('app/galleries/index') ?>" method="get" class="flex items-center gap-3">
                    <div class="flex items-center gap-2">
                        <span class="text-sm text-gray-600 whitespace-nowrap">Tampilkan:</span>
                        <select name="per_page" class="border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" onchange="this.form.submit();">
                    ...
                    <!-- MAGIC: Otomatis preserve SEMUA parameter (q, poststatus, category, date, dll) -->
...
<script>
    let selectedIds = JSON.parse(sessionStorage.getItem('selectedGalleryIds')) || [];

    function updateSelectedIds() {
        console.log('Updating selectedIds:', selectedIds);
        sessionStorage.setItem('selectedGalleryIds', JSON.stringify(selectedIds));
        toggleDeleteButton();
    }
    ...
    function deleteSelected() {
        ...
        if (confirm('Yakin ingin menghapus ' + selectedIds.length + ' data terpilih?')) {
            ...
            sessionStorage.removeItem('selectedGalleryIds');
        }
    }
