
<div class="section py-5 bg-green2">
    <div class="container">
        <div class="row text-center-xs">

            <div class="col-12 col-md-6">
                <h3 class="m-0">Selalu Terhubung! Akses Dokumen Darimana saja! </h3>
                <p class="m-0">Anda akan selalu mendapatkan berita terbaru pertama kali!</p>
            </div>

            <div class="col-12 mb-5 d-block d-md-none"><!-- mobile spacer --></div>

            <div class="col-12 col-md-6 text-align-end text-center-xs">
                <!-- App Store -->

                <div class="mt-3 d-block d-md-none"><!-- mobile spacer --></div>
                <div class="mt-3 d-block d-md-none"><!-- mobile spacer --></div>

                <!-- Play Store -->
                <a href="#" class="btn btn-dark bg-gradient-dark border-0 mx-1 rounded-xl transition-hover-top">
                    <i class="float-start">
                        <svg height="44px" id="Layer_1" style="enable-background:new 0 0 58.282 63.725;" version="1.1" viewBox="0 0 58.282 63.725" width="44px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Google_Play_Shape"><g><path d="M0.257,3.012C0.106,3.547,0,4.121,0,4.779v54.172c0,0.649,0.102,1.22,0.25,1.748l30.378-28.805    L0.257,3.012z" style="fill:#48A0DC;"/></g></g><g id="Google_Play_Shape_5_"><g><path d="M7.016,0.908C5.602,0.045,4.208-0.171,3.031,0.127l30.505,29.01l8.948-8.483L7.016,0.908z" style="fill:#88C057;"/></g></g><g id="Google_Play_Shape_2_"><g><path d="M55.56,27.934l-9.468-5.271l-9.696,9.194l9.878,9.396l9.213-5.063    C59.98,33.627,58.339,29.497,55.56,27.934z" style="fill:#FFCC66;"/></g></g><g id="Google_Play_Shape_3_"><g><path d="M33.487,34.613L2.944,63.571c1.195,0.335,2.621,0.136,4.071-0.749l35.579-19.549L33.487,34.613z" style="fill:#ED7161;"/></g></g><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/></svg>

                    </i>

                    <span class="float-start text-align-start px-2">
                        <span class="d-block lh-1 small">Unduh di </span>
                        <span class="d-block lh-1 pt-1">Play Store</span>
                    </span>
                </a>

            </div>

        </div>
    </div>
</div>
<!-- /CALL TO ACTION : APP -->



<footer id="footer" class="bg-green2 text-white">
    <!-- line divider -->
    <hr class="bg-gray-600 fw-medium my-0 container">
        <div class="container py-5">

            <div class="row g-4">


                <div class="col-12 col-lg-4 mb-4 order-1 fw-medium">

                    <h4 class="mb-4 fw-medium ">GET IN TOUCH</h4>
                    <!-- logo -->
                    <img src="<?php echo site_url($this->config->item('site_logo2')) ?>" width="110" alt="<?php echo $this->config->item('site_name') ?>">



                        <p class="col-md-10 mt-3">
                            <?php echo $this->config->item('site_name') ?>
                        </p>

                        <div class="small d-flex align-items-start my-3 ">
                            <svg class="me-3 flex-none my-1" height="24px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                                <circle cx="12" cy="10" r="3"></circle>
                            </svg>
                            <ul class="list-unstyled p-0 m-0 w-100">
                                <li><a class="text-dark-emphasis-500 text-dashed"><?php echo $this->config->item('site_address') ?></a></li>
                            </ul>
                        </div>

                        <div class="small d-flex align-items-start my-3">
                            <svg class="me-3 flex-none my-1" height="24px" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"></path>
                            </svg>
                            <ul class="list-unstyled p-0 m-0 w-200">
                                <li><a href="mailto:<?php echo $this->config->item('site_email') ?>" class="text-dark-emphasis-500 text-dashed"><?php echo $this->config->item('site_email') ?></a></li>
                            </ul>
                        </div>
                        <div class="small d-flex align-items-start my-3">
                            <svg class="me-3 flex-none my-1" height="20px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                <path d="m17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52s-.669-1.612-.916-2.207c-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413s.248-1.289.173-1.413c-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1 -5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1 -1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297a11.815 11.815 0 0 0 -8.413-3.488c-6.555 0-11.89 5.335-11.893 11.892 0 2.096.547 4.142 1.588 5.945l-1.688 6.163 6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0 -3.48-8.413z"></path>
                            </svg>
                            <ul class="list-unstyled p-0 m-0 w-200">
                                <li><a href="tel:<?php echo $this->config->item('site_phone') ?>" class="text-dark-emphasis-500 text-dashed"><?php echo $this->config->item('site_phone') ?></a></li>
                            </ul>
                        </div>
                        <div class="my-4"><!-- copyright -->
                            <p class="small mb-0">&copy; <?php echo $this->config->item('site_name') ?></p>
                            <p class="small mb-0">All rights reserved.</p>
                        </div>

                        <!-- social icons -->
                        <a href="<?php echo $this->config->item('social_facebook') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="facebook page">
                            <svg height="44px" id="Layer_1" style="enable-background:new 0 0 64 63.999;" version="1.1" viewBox="0 0 64 63.999" width="44px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Shape_74_"><g><circle cx="32" cy="32" r="32" style="fill:#48A0DC;"/></g></g><g id="Facebook_Logo"><g><path d="M35.896,22.028h4.125v-6.093h-4.848v0.022c-5.872,0.208-7.078,3.51-7.184,6.979h-0.011v3.04h-3.999    v5.968h3.999v15.991h6.029V31.944h4.938l0.951-5.968h-5.889V24.14C34.007,22.97,34.785,22.028,35.896,22.028z" style="fill:#FFFFFF;"/></g></g><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/></svg>
                        </a>

                        <a href="<?php echo $this->config->item('social_twitter') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="twitter page">
                            <svg height="44px" width="44px" id="Layer_1" style="enable-background:new 0 0 1000 1000;" version="1.1" viewBox="0 0 1000 1000" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><style type="text/css">
                                    .st0{
                                        fill:#1DA1F2;
                                    }
                                    .st1{
                                        fill:#FFFFFF;
                                    }
                                    .st2{
                                        fill:none;
                                    }
                                </style><title/><g><g id="Dark_Blue"><path class="st0" d="M500,0L500,0c276.1,0,500,223.9,500,500v0c0,276.1-223.9,500-500,500h0C223.9,1000,0,776.1,0,500v0    C0,223.9,223.9,0,500,0z"/></g><g id="Logo_FIXED"><path class="st1" d="M384,754c235.8,0,364.9-195.4,364.9-364.9c0-5.5,0-11.1-0.4-16.6c25.1-18.2,46.8-40.6,64-66.4    c-23.4,10.4-48.2,17.2-73.6,20.2c26.8-16,46.8-41.2,56.4-70.9c-25.2,14.9-52.7,25.5-81.4,31.1c-48.6-51.6-129.8-54.1-181.4-5.6    c-33.3,31.3-47.4,78-37.1,122.5c-103.1-5.2-199.2-53.9-264.3-134c-34,58.6-16.7,133.5,39.7,171.2c-20.4-0.6-40.4-6.1-58.2-16    c0,0.5,0,1.1,0,1.6c0,61,43,113.6,102.9,125.7c-18.9,5.1-38.7,5.9-57.9,2.2c16.8,52.2,64.9,88,119.8,89.1    c-45.4,35.7-101.5,55.1-159.2,55c-10.2,0-20.4-0.6-30.5-1.9C246.1,734,314.4,754,384,753.9"/><path class="st2" d="M500,0L500,0c276.1,0,500,223.9,500,500v0c0,276.1-223.9,500-500,500h0C223.9,1000,0,776.1,0,500v0    C0,223.9,223.9,0,500,0z"/></g></g></svg>
                        </a>

                        <a href="<?php echo $this->config->item('social_linkedin') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="linkedin page">
                            <svg height="44px" id="Layer_1" style="enable-background:new 0 0 64 63.999;" version="1.1" viewBox="0 0 64 63.999" width="44px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Shape_66_"><g><circle cx="32" cy="32" r="32" style="fill:#48A0DC;"/></g></g><g id="Linked_IN_logo"><g><path d="M18.031,43.968h5.993V23.993h-5.993V43.968z M41.375,23.304    c-2.909,0-5.511,1.062-7.356,3.406v-2.779h-6.014v20.037h6.014V33.132c0-2.289,2.098-4.524,4.728-4.524    c2.629,0,3.274,2.235,3.274,4.471v10.89h5.993V32.63C48.014,24.754,44.285,23.304,41.375,23.304z M21,21.999c1.657,0,3-1.344,3-3    c0-1.657-1.343-3-3-3c-1.657,0-3,1.343-3,3C18,20.655,19.343,21.999,21,21.999z" style="fill-rule:evenodd;clip-rule:evenodd;fill:#FFFFFF;"/></g></g><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/></svg>
                        </a>

                        <a href="<?php echo $this->config->item('social_youtube') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="youtube page">
                            <svg height="44px" id="Layer_1" style="enable-background:new 0 0 64.002 63.996;" version="1.1" viewBox="0 0 64.002 63.996" width="44px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Shape_57_"><g><circle cx="32.001" cy="31.998" r="31.998" style="fill:#ED7161;"/></g></g><g id="YouTube_Logo"><g><path d="M37.925,25.993c0.354,0,0.72-0.204,1.096-0.409    c0.379-0.208,0.742-0.511,1.088-0.907v1.077h1.89v-9.728h-1.89v7.385c-0.176,0.211-0.371,0.385-0.581,0.521    c-0.215,0.137-0.391,0.205-0.529,0.205c-0.177,0-0.307-0.051-0.384-0.156c-0.078-0.103-0.122-0.268-0.122-0.496v-7.459h-1.887    v8.13c0,0.58,0.111,1.013,0.331,1.3C37.159,25.749,37.486,25.993,37.925,25.993z M31.229,25.999c1.013,0,1.807-0.322,2.386-0.779    c0.575-0.459,0.863-1.088,0.863-1.891v-4.851c0-0.716-0.295-1.304-0.88-1.759c-0.591-0.451-1.346-0.68-2.269-0.68    c-1.012,0-1.819,0.216-2.418,0.646c-0.599,0.432-0.898,1.008-0.898,1.738v4.867c0,0.8,0.293,1.432,0.876,1.905    C29.474,25.668,30.255,25.999,31.229,25.999z M30.172,18.657c0-0.184,0.095-0.332,0.283-0.449c0.193-0.113,0.44-0.168,0.745-0.168    c0.331,0,0.599,0.055,0.803,0.168c0.203,0.117,0.306,0.266,0.306,0.449v4.611c0,0.228-0.102,0.406-0.301,0.537    c-0.2,0.13-0.469,0.193-0.808,0.193c-0.33,0-0.585-0.062-0.763-0.191c-0.177-0.128-0.266-0.306-0.266-0.539V18.657z M22.845,26.02    h2.387v-5.812l2.778-8.2h-2.425l-1.478,5.602h-0.15l-1.551-5.602h-2.403l2.842,8.462V26.02z M43.422,30.014H20.58    c-3.631,0-6.574,2.83-6.574,6.315v5.342c0,3.487,2.943,6.316,6.574,6.316h22.842c3.63,0,6.574-2.829,6.574-6.316v-5.342    C49.996,32.844,47.052,30.014,43.422,30.014z M23.98,35.433h-2.021v8.363h-1.953v-8.363h-2.021v-1.424h5.996V35.433z     M30.006,43.913H27.9v-0.865c-0.392,0.319-0.795,0.564-1.217,0.729c-0.42,0.172-0.827,0.253-1.224,0.253    c-0.488,0-0.852-0.117-1.101-0.354c-0.244-0.234-0.369-0.586-0.369-1.059v-6.603h2.105v6.06c0,0.188,0.046,0.32,0.13,0.404    c0.092,0.086,0.237,0.125,0.435,0.125c0.151,0,0.35-0.055,0.586-0.165c0.239-0.111,0.456-0.253,0.654-0.424v-6h2.105V43.913z     M37.979,42.41c0,0.517-0.163,0.913-0.489,1.19c-0.323,0.276-0.798,0.413-1.413,0.413c-0.409,0-0.772-0.052-1.089-0.16    c-0.318-0.104-0.617-0.269-0.89-0.495v0.562h-2.103v-9.911h2.103V37.2c0.282-0.221,0.581-0.391,0.892-0.507    c0.318-0.117,0.638-0.173,0.961-0.173c0.658,0,1.16,0.153,1.507,0.464c0.349,0.313,0.521,0.77,0.521,1.367V42.41z M45.993,40.181    h-3.882v1.427c0,0.398,0.064,0.674,0.197,0.83c0.136,0.154,0.365,0.23,0.688,0.23c0.339,0,0.572-0.065,0.707-0.196    c0.132-0.133,0.202-0.419,0.202-0.864v-0.345h2.088v0.389c0,0.775-0.253,1.361-0.767,1.756c-0.505,0.391-1.265,0.584-2.276,0.584    c-0.909,0-1.627-0.206-2.147-0.623c-0.521-0.412-0.785-0.987-0.785-1.717v-3.403c0-0.654,0.289-1.192,0.864-1.604    c0.574-0.415,1.312-0.62,2.222-0.62c0.929,0,1.643,0.19,2.142,0.573c0.499,0.385,0.748,0.934,0.748,1.651V40.181z M34.969,37.681    c-0.148,0-0.297,0.023-0.441,0.071c-0.146,0.046-0.288,0.124-0.43,0.223v4.555c0.168,0.119,0.333,0.209,0.493,0.261    c0.161,0.05,0.326,0.078,0.503,0.078c0.255,0,0.441-0.051,0.562-0.156c0.118-0.103,0.179-0.271,0.179-0.502v-3.777    c0-0.248-0.07-0.435-0.217-0.562C35.465,37.745,35.248,37.681,34.969,37.681z" style="fill-rule:evenodd;clip-rule:evenodd;fill:#FFFFFF;"/></g></g><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/></svg>
                        </a>
                        <a href="<?php echo $this->config->item('social_instagram') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="instagram page">
                            <svg height="44px" width="44px"   enable-background="new 0 0 24 24" id="Layer_1" version="1.1" viewBox="0 0 24 24" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g><path d="M17,22H7c-2.7614236,0-5-2.2385769-5-5V7c0-2.7614236,2.2385764-5,5-5h10c2.7614231,0,5,2.2385764,5,5v10   C22,19.7614231,19.7614231,22,17,22z" fill="#FFFFFF"/><radialGradient cx="2.4583333" cy="22.208334" gradientUnits="userSpaceOnUse" id="SVGID_1_" r="26.8152752"><stop offset="0" style="stop-color:#FFD35A"/><stop offset="0.2548544" style="stop-color:#F7964C"/><stop offset="0.5995145" style="stop-color:#F05B70"/><stop offset="0.7174556" style="stop-color:#BD6186"/><stop offset="0.8532288" style="stop-color:#85659B"/><stop offset="0.9509482" style="stop-color:#5C66A9"/><stop offset="1" style="stop-color:#4766B0"/></radialGradient><path d="M17.0219727,1H6.9780273C3.6816406,1,1,3.6816406,1,6.9780273v10.0439453   C1,20.3183594,3.6816406,23,6.9780273,23h10.0439453C20.3183594,23,23,20.3183594,23,17.0219727V6.9780273   C23,3.6816406,20.3183594,1,17.0219727,1z M21,17.0219727C21,19.215332,19.215332,21,17.0219727,21H6.9780273   C4.784668,21,3,19.215332,3,17.0219727V6.9780273C3,4.784668,4.784668,3,6.9780273,3h10.0439453   C19.215332,3,21,4.784668,21,6.9780273V17.0219727z M12,6c-3.3085938,0-6,2.6914063-6,6s2.6914063,6,6,6s6-2.6914063,6-6   S15.3085938,6,12,6z M12,16c-2.2055664,0-4-1.7944336-4-4s1.7944336-4,4-4s4,1.7944336,4,4S14.2055664,16,12,16z M18,5   c-0.5512695,0-1,0.4487305-1,1s0.4487305,1,1,1s1-0.4487305,1-1S18.5512695,5,18,5z" fill="url(#SVGID_1_)"/><linearGradient gradientUnits="userSpaceOnUse" id="SVGID_2_" x1="0.5337821" x2="23.466217" y1="6.6532149" y2="17.3467846"><stop offset="0" style="stop-color:#FFFFFF;stop-opacity:0.2"/><stop offset="1" style="stop-color:#FFFFFF;stop-opacity:0"/></linearGradient><path d="M17.0219727,1H6.9780273C3.6816406,1,1,3.6816406,1,6.9780273v10.0439453   C1,20.3183594,3.6816406,23,6.9780273,23h10.0439453C20.3183594,23,23,20.3183594,23,17.0219727V6.9780273   C23,3.6816406,20.3183594,1,17.0219727,1z" fill="url(#SVGID_2_)"/></g><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/><g/></svg>
                        </a>
                        <a href="<?php echo $this->config->item('social_tiktok') ?>" class="btn btn-sm text-white transition-hover-top ms-0 p-1" rel="noopener" aria-label="tiktok page">
                            <svg height="44px" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd" version="1.1" viewBox="0 0 512 512" width="44px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xodm="http://www.corel.com/coreldraw/odm/2003"><defs><style type="text/css">
                                        <![CDATA[
                                        .fil1 {
                                            fill:white
                                        }
                                        .fil0 {
                                            fill:url(#id0)
                                        }
                                        ]]>
                                    </style><linearGradient gradientUnits="userSpaceOnUse" id="id0" x1="67.83" x2="474.19" y1="82.42" y2="389.98"><stop offset="0" style="stop-opacity:1; stop-color:#67C4CE"/><stop offset="1" style="stop-opacity:1; stop-color:#E62A58"/></linearGradient></defs><g id="Layer_x0020_1"><g id="_2515485150816"><path class="fil0" d="M256 0c141.39,0 256,114.61 256,256 0,141.39 -114.61,256 -256,256 -141.39,0 -256,-114.61 -256,-256 0,-141.39 114.61,-256 256,-256z"/><path class="fil1" d="M313.5 106.01c0.01,4.58 1.36,70.83 70.87,74.96 0,19.1 0.02,32.95 0.02,51.18 -5.26,0.3 -45.76,-2.64 -70.97,-25.12l-0.08 99.64c0.96,69.16 -49.93,111.24 -116.46,96.7 -114.71,-34.31 -76.59,-204.44 38.59,-186.24 0,54.93 0.03,-0.01 0.03,54.93 -47.58,-7 -63.5,32.58 -50.85,60.93 11.5,25.8 58.88,31.39 75.41,-5.01 1.87,-7.12 2.8,-15.25 2.8,-24.37l0 -197.85 50.64 0.25z"/></g></g></svg>
                        </a>

                </div>


                <div class="col-12 col-lg-3 mb-4 order-last">

                    <?php if ($this->config->item('footermenu')) { ?>
                        <h4 class="mb-4">Pintasan</h4>

                        <ul class="list-unstyled">
                            <?php
                            $conffootermenu = $this->config->item('footermenu');
                            $fmenu = json_decode($conffootermenu->menu);
                            foreach ($fmenu as $fm) {
                                $tmpfmenu = explode(";", $fm->id);
                                echo '<li class = "list-item mb-2 fw-medium"><i class = "fi fi-arrow-end me-2 smaller"></i>';
                                echo '<a class = "text-dark-emphasis-500" href = "' . $tmpfmenu[1] . '">' . $tmpfmenu[0] . '</a></li>';
                            }
                        }
                        ?>
                    </ul>

                </div>


                <div class="col-12 col-lg-5 mb-4 text-dark order-3">

                    <div class="card">
                        <div class="card-header text-center">
                            <h6>Trafik Pengunjung</h6>
                        </div>
                        <div class="card-body text-center">
                            <div class="row g-4">
                                <div class="col-6 col-md-3">
                                    <div class="h6 ">
                                        <span id="vdaily" class="alert alert-mini rounded-pill bg-gradient-warning text-dark p-1">0</span>
                                    </div>
                                    <h2 class="h6 m-0">Hari ini</h2>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="h6 ">
                                        <span <span id="vmonthly" class="alert alert-mini rounded-pill bg-gradient-warning text-dark p-1">0</span>
                                    </div>
                                    <h2 class="h6 m-0">Bulan ini</h2>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="h6 ">
                                        <span id="vtotal" class="alert alert-mini rounded-pill bg-gradient-warning text-dark p-1">0</span>
                                    </div>
                                    <h2 class="h6 m-0">Total Klik</h2>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="h6 ">
                                        <span id="vunique" class="alert alert-mini rounded-pill bg-gradient-warning text-dark p-1">0</span>
                                    </div>
                                    <h2 class="h6 m-0">Visitor Unik</h2>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="card mt-1">
                        <div class="card-header text-center">
                            <h6>Mohon Penilaian Anda Terhadap Layanan Kami</h6>
                        </div>

                        <div class="card-body">

                            <div class="row g-4 col-border text-center text-gray-700">
                                <div class="col-6 col-md-3 text-dark-hover" onclick="submit_survey(1)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="66" height="66" fill="#f0377b" class="bi bi-emoji-heart-eyes" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                                        <path d="M11.315 10.014a.5.5 0 0 1 .548.736A4.498 4.498 0 0 1 7.965 13a4.498 4.498 0 0 1-3.898-2.25.5.5 0 0 1 .548-.736h.005l.017.005.067.015.252.055c.215.046.515.108.857.169.693.124 1.522.242 2.152.242.63 0 1.46-.118 2.152-.242a26.58 26.58 0 0 0 1.109-.224l.067-.015.017-.004.005-.002zM4.756 4.566c.763-1.424 4.02-.12.952 3.434-4.496-1.596-2.35-4.298-.952-3.434m6.488 0c1.398-.864 3.544 1.838-.952 3.434-3.067-3.554.19-4.858.952-3.434"/>
                                    </svg>
                                    <h2 class="h6 m-0">Sangat Puas</h2>
                                </div>

                                <div class="col-6 col-md-3 text-dark-hover" onclick="submit_survey(2)">

                                    <svg xmlns="http://www.w3.org/2000/svg" width="66" height="66" fill="#14cc30" class="bi bi-emoji-smile" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                                        <path d="M4.285 9.567a.5.5 0 0 1 .683.183A3.498 3.498 0 0 0 8 11.5a3.498 3.498 0 0 0 3.032-1.75.5.5 0 1 1 .866.5A4.498 4.498 0 0 1 8 12.5a4.498 4.498 0 0 1-3.898-2.25.5.5 0 0 1 .183-.683M7 6.5C7 7.328 6.552 8 6 8s-1-.672-1-1.5S5.448 5 6 5s1 .672 1 1.5m4 0c0 .828-.448 1.5-1 1.5s-1-.672-1-1.5S9.448 5 10 5s1 .672 1 1.5"/>
                                    </svg>

                                    <h2 class="h6 m-0">Puas</h2>
                                </div>

                                <div class="col-6 col-md-3 text-dark-hover" onclick="submit_survey(3)">

                                    <svg xmlns="http://www.w3.org/2000/svg" width="66" height="66" fill="#6fab78" class="bi bi-emoji-grimace" viewBox="0 0 16 16">
                                        <path d="M7 6.25c0 .69-.448 1.25-1 1.25s-1-.56-1-1.25S5.448 5 6 5s1 .56 1 1.25m3 1.25c.552 0 1-.56 1-1.25S10.552 5 10 5s-1 .56-1 1.25.448 1.25 1 1.25m2.98 3.25A1.5 1.5 0 0 1 11.5 12h-7a1.5 1.5 0 0 1-1.48-1.747v-.003A1.5 1.5 0 0 1 4.5 9h7a1.5 1.5 0 0 1 1.48 1.747zm-8.48.75h.25v-.75H3.531a1 1 0 0 0 .969.75m7 0a1 1 0 0 0 .969-.75H11.25v.75zm.969-1.25a1 1 0 0 0-.969-.75h-.25v.75zM4.5 9.5a1 1 0 0 0-.969.75H4.75V9.5zm1.75 2v-.75h-1v.75zm.5 0h1v-.75h-1zm1.5 0h1v-.75h-1zm1.5 0h1v-.75h-1zm1-2h-1v.75h1zm-1.5 0h-1v.75h1zm-1.5 0h-1v.75h1zm-1.5 0h-1v.75h1z"/>
                                        <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16m0-1A7 7 0 1 1 8 1a7 7 0 0 1 0 14"/>
                                    </svg>
                                    <h2 class="h6 m-0">Cukup</h2>
                                </div>

                                <div class="col-6 col-md-3 text-dark-hover" onclick="submit_survey(4)">

                                    <svg xmlns="http://www.w3.org/2000/svg" width="66" height="66" fill="#f5aa42" class="bi bi-emoji-frown" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                                        <path d="M4.285 12.433a.5.5 0 0 0 .683-.183A3.498 3.498 0 0 1 8 10.5c1.295 0 2.426.703 3.032 1.75a.5.5 0 0 0 .866-.5A4.498 4.498 0 0 0 8 9.5a4.5 4.5 0 0 0-3.898 2.25.5.5 0 0 0 .183.683M7 6.5C7 7.328 6.552 8 6 8s-1-.672-1-1.5S5.448 5 6 5s1 .672 1 1.5m4 0c0 .828-.448 1.5-1 1.5s-1-.672-1-1.5S9.448 5 10 5s1 .672 1 1.5"/>
                                    </svg>
                                    <h2 class="h6 m-0">Tidak Puas</h2>
                                </div>

                                <div class="col-12 col-lg-12" id="surveymessage">
                                    <span id="classmessage" class="text-wrap"></span>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>

        </div>

</footer>
<!-- /Footer -->


</div><!-- /#wrapper -->

<script src="<?php echo site_url() ?>assets/web/js/core.min.js"></script>
<?php
if (isset($extrajs)) {
    $this->load->view($extrajs);
}
?>
<script type="text/javascript">
                                    jQuery(document).ready(function () {
                                        setTimeout(function () {
                                            $.ajax({
                                                ype: "GET",
                                                url: '<?php echo site_url("web/Web/ajax_get_visitor_counter") ?>',
                                                dataType: 'JSON',
                                                async: false,
                                                success: function (data) {
                                                    if (data.status === "success") {
                                                        $("#vunique").html(data.vunique.vunique);
                                                        $("#vtotal").html(data.vtotal.visitor);
                                                        $("#vmonthly").html(data.vmonthly.visitor);
                                                        $("#vdaily").html(data.vdaily.visitor);
                                                    }
                                                },
                                                error: function (error) {
                                                    console.log("cannot send like");
                                                }
                                            });
                                        }, 2000);

                                        $("#surveymessage").hide();

                                    });

                                    function submit_survey(value) {
                                        $.ajax({
                                            type: "GET",
                                            url: "<?php echo site_url("web/surveys/submit/") ?>" + value,
                                            dataType: 'JSON',
                                            async: false,
                                            success: function (data) {
                                                if (data.status === "success") {
                                                    $("#classmessage").attr('class', 'alert alert-mini alert-success text-wrap');
                                                    $("#classmessage").html('Terima kasih sudah melakukan survey');
                                                    $("#surveymessage").show();
                                                    setTimeout(function () {
                                                        $("#surveymessage").hide();
                                                    }, 5000);
                                                } else {
                                                    $("#classmessage").attr('class', 'alert alert-mini alert-danger text-wrap');
                                                    $("#classmessage").html('Tidak dapat mengirim survey');
                                                    $("#surveymessage").show();
                                                    setTimeout(function () {
                                                        $("#surveymessage").hide();
                                                    }, 5000);
                                                }
                                            },
                                            error: function (error) {
                                                $("#classmessage").attr('class', 'alert alert-mini alert-danger text-wrap');
                                                $("#classmessage").html('Tidak dapat mengirim survey');
                                                $("#surveymessage").show();
                                                setTimeout(function () {
                                                    $("#surveymessage").hide();
                                                }, 5000);
                                            }
                                        });
                                    }


</script>
</body>
</html>
